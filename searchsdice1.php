...
$result=mysqli_query($koneksi,$query);
	...
	// tampilkan kotak pencarian dan jumlah hasil pencarian
	  ...
	  echo "<br /><br><table><tr><td></td><td><form action='searchsdice1.php' method='GET'><input type='text' onclick=value='' size='50' name='keyword' value='$keyword' style='width: 500px; height: 30px; font-size: 16px;'> <input type='submit' value='Search'></form></td></tr></table>";
...
$query= "SELECT* FROM (SELECT g.id id,kata, A, jmlkatakunci, B, jmlkatadok, jmlkatadok-A C, 2*A/(2*A+B+(jmlkatadok-A)) dice FROM (SELECT *,jmlkatakunci-A B FROM (SELECT y.id,y.kata,y.freq A FROM (SELECT * FROM katakunci) AS x JOIN (SELECT * FROM token WHERE id!='0' ORDER BY id+0) as Y ON x.kata=y.kata) AS d JOIN (SELECT COUNT(kata) jmlkatakunci FROM katakunci) AS e) AS f JOIN (SELECT id,COUNT(kata) jmlkatadok FROM token GROUP BY id) AS g ON f.id=g.id GROUP BY g.id ORDER BY dice DESC) AS r join (SELECT * FROM berita) AS t ON r.id=t.id";
//SELECT g.id id,kata, A, jmlkatakunci, B, jmlkatadok, jmlkatadok-A C, 2*A/(2*A+B+(jmlkatadok-A)) dice FROM (SELECT *,jmlkatakunci-A B FROM (SELECT y.id,y.kata,y.freq A FROM (SELECT * FROM katakunci WHERE id='1') AS x JOIN (SELECT * FROM token WHERE id!='0' ORDER BY id+0) as Y ON x.kata=y.kata) AS d JOIN (SELECT COUNT(kata) jmlkatakunci FROM katakunci) AS e) AS f JOIN (SELECT id,COUNT(kata) jmlkatadok FROM token GROUP BY id) AS g ON f.id=g.id GROUP BY g.id ORDER BY dice DESC";
...
echo "<tr><td><font color=black>" . " $row[url] " . "</font></td></tr>"; 
...
echo "<tr><td><font color=black>" . " Dice : $row[dice] " . "</font></td></tr>"; 
echo "<tr><td><br></td></tr>";
